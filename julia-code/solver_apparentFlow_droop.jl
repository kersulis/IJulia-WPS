function solver_apparentFlow_droop(psDL)
    ...
    for idx = 1:length(f)
        (m, ρ, θ, α, PowerBalance, Slack, Congestion, Participation) = droopApparentModel(idx, 1, Rp, Gp, Dp, f, t, x, Y, bustype, Plim,k,Vg)
        status = solve(m);
...
function droopApparentModel(idx, sense, Rp, Gp, Dp, f, t, x, Y, bustype, Plim,k, Vg)
    # DROOP RESPONSE
    ...
    Ei = Vg[f[idx]]*exp((θ[f[idx]]*im.0)) # voltage at node i
    Ek = Vg[t[idx]]*exp((θ[t[idx]]*im.0)) # voltage at node k
    Yik = 1/complex(r[idx],x[idx]) # real and imaginary parts

    Sik = abs(Ei*(conj(Ei - Ek))/Yik) # apparent power magnitude

    @addConstraint(m, Congestion, Sik == sense*Plim[idx]) # saturate a line
