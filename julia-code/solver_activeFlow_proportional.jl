function solver_activeFlow_proportional(psDL)
    ...
    for idx = 1:length(f)
        (m, ρ, θ, α, PowerBalance, Slack, Congestion, Participation) = proportionalDCModel(idx, 1, Rp, Gp, Dp, f, t, x, Y, bustype, Plim)
        status = solve(m);
...
function proportionalDCModel(idx, sense, Rp, Gp, Dp, f, t, x, Y, bustype, Plim)
